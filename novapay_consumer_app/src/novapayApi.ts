...
type SurfaceList = Readonly<{
  ...
  trust?: readonly unknown[];
  role_permissions?: readonly string[];
}>;

export type ConsumerSurfaceState = Readonly<{
  status: BackendState;
  apiUrl: string;
  apps?: SurfaceList;
  portals?: SurfaceList;
...
export async function loadConsumerSurface(): Promise<ConsumerSurfaceState> {
  const apiUrl = resolveNovaPayApiUrl();
  try {
    const [apps, portals] = await Promise.all([fetchJson("/v1/novapay/apps"), fetchJson("/v1/novapay/portals")]);
    return {
      status: "loaded",
      apiUrl,
      apps,
