...
export function DesignStudioWorkspace({ session, baseUrl = "", onNavigate }) {
  ...
  return <div className="studio-workspace" data-testid="design-studio-workspace">
    ...
    <nav className="studio-tabs" aria-label="Design workspace tabs">{TABS.map((name) => <button key={name} type="button" className={tab === name ? "active" : ""} onClick={() => setTab(name)}>{name}</button>)}</nav>
