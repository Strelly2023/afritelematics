...
class MemoryReplayRepository(ReplayPlanRepository):
    ...
    transitions: dict[tuple[str, str], list[ReplayTransitionRecord]] = field(
    ...
    async def list(
        ...
        values = [
            deepcopy(plan)
            for (item_tenant, _), plan in self.plans.items()
    ...
    async def get_approvals(self, *, tenant_id: str, plan_id: str) -> list[ReplayApprovalRecord]:
        return [
            deepcopy(item)
            for (item_tenant, _), item in self.approvals.items()
            if item_tenant == tenant_id and item.replay_plan_id == plan_id
        ]
        ...
        *,
        ...
        consumed = ReplayApprovalRecord(
            **{
                **asdict(approval),
                "status": "CONSUMED",
                "consumed_at": approval.created_at,
    ...
    async def append_transition(self, transition: ReplayTransitionRecord) -> ReplayTransitionRecord:
        self.transitions[(transition.tenant_id, transition.replay_plan_id)].append(
            deepcopy(transition)
        )
