...
class NovaRideRuntimeSettings:
    ...
    def from_env(cls, env: Mapping[str, str] | None = None) -> "NovaRideRuntimeSettings":
        ...
        topics = {
            ...
            "resilience": values.get("NOVARIDE_KAFKA_TOPIC_RESILIENCE", "novaride.resilience.events"),
            "mobile_sync": values.get("NOVARIDE_KAFKA_TOPIC_MOBILE_SYNC", "novaride.mobile.sync.events"),
            "provider_health": values.get("NOVARIDE_KAFKA_TOPIC_PROVIDER_HEALTH", "novaride.provider.health.events"),
            ...
            "deadletter": values.get("NOVARIDE_KAFKA_TOPIC_DEADLETTER", "novaride.deadletter.events"),
        }
        settings = cls(
            ...
            provider_probe_interval_seconds=int(values.get("NOVARIDE_PROVIDER_PROBE_INTERVAL_SECONDS", "30")),
            provider_probe_timeout_seconds=float(values.get("NOVARIDE_PROVIDER_PROBE_TIMEOUT_SECONDS", "2.0")),
            outbox_poll_interval_seconds=float(values.get("NOVARIDE_OUTBOX_POLL_INTERVAL_SECONDS", "1.0")),
            ...
            retention_offline_operations_days=int(values.get("NOVARIDE_RETENTION_OFFLINE_OPERATIONS_DAYS", "30")),
            ...
            emergency_mode_enabled=values.get("NOVARIDE_EMERGENCY_MODE_ENABLED", "false").lower() == "true",
            feature_flags={
                "real_payments": values.get("NOVARIDE_REAL_PAYMENTS_ENABLED", "false").lower() == "true",
...
def create_runtime_from_settings(settings: NovaRideRuntimeSettings) -> NovaRideRuntime:
    settings.validate()
    if settings.environment == RuntimeEnvironment.PRODUCTION and settings.persistence_adapter == PersistenceAdapter.MEMORY:
