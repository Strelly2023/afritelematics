# afritech/speculative/civilization/civilization_engine.py
...
class CivilizationEngine:
    ...
    This class:
    - Orchestrates execution and verification
    - NEVER owns execution authority
    - NEVER bypasses the execution kernel
    """
    ...
    def process(
        ...
        *,
        ...
        if not isinstance(context, RuntimeContext):
            raise CivilizationError("Invalid RuntimeContext")
        ...
        # -------------------------------------------------------------
        # STEP 1 — CONSTITUTIONAL EXECUTION (KERNEL-ENFORCED)
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # STEP 2 — DISTRIBUTED VERIFICATION (DECISION ONLY)
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # STEP 3 — FEDERATED VERIFICATION (OPTIONAL)
        # -------------------------------------------------------------
        ...
        self._reputation.update_from_proof(distributed_proof)

        # -------------------------------------------------------------
        # STEP 5 — APPLY ECONOMICS
        # -------------------------------------------------------------

        self._economics.apply_node_consensus(distributed_proof)

        if federated_proof is not None:
            self._economics.apply_federation_consensus(federated_proof)
    ...
    def _distributed_verify(
        ...
        *,
        ...
        """
        Deterministic distributed verification.

        NOTE:
        - This step NEVER re-executes
        - It only verifies execution artifacts
        """
        ...
        return self._distributed_verifier.verify(node_results)
    ...
    def _federated_verify(
        ...
        """
        Federation verification is best-effort.
        Failure does not invalidate execution.
        """

        try:
            system_results = self._build_system_results(distributed_proof)
            return self._federation_verifier.verify(system_results)
        except Exception:
            return None
    ...
    def _build_system_metrics(
        ...
        disagreement_count = len(
            distributed_proof.get("disagreeing_nodes", [])
        )
        total_nodes = max(
            distributed_proof.get("node_count", 1),
            1,
        )
