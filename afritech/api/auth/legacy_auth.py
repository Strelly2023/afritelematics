...
class Response(dict):
    """Minimal response object for legacy auth decorators without DRF."""
...
def _get_api_key_model():
    ...
    except ModuleNotFoundError as exc:  # pragma: no cover - dependency is installed in production
        raise RuntimeError("Django is not installed") from exc
    ...
    if not apps.ready:
        try:
            ...
            raise RuntimeError(
                f"Unable to initialize Django settings module {settings_module!r}"
            ) from exc
...
def require_api_key(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        try:
            ...
            # ✅ HASHED KEY LOOKUP
            # =====================================================
            api_key_model = _get_api_key_model()
            key_hash = api_key_model.hash_key(raw_key)

            try:
                api_key = api_key_model.objects.get(key_hash=key_hash)
