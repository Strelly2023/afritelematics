...
| Requirement ID | Capability | Baseline status | Final status | Missing implementation | Validation command | Evidence path | Blocker |
| --- | --- | --- | --- | --- | --- | --- | --- |
| NPY-PG-001 | Normalized PostgreSQL monetary persistence | Partially implemented | VERIFIED | PostgreSQL repository now emits normalized columns for wallet, account, ledger, settlement, refund, reconciliation, and outbox rows. | `python3 -m pytest -q afritech/tests/novapay/test_postgres_repository.py afritech/tests/novapay/test_ecosystem.py` | `artifacts/phase1-core-engineering/novapay/` |  |
| NPY-LEDGER-001 | Transactional double-entry ledger | Missing | VERIFIED | Transfer posting now runs inside an explicit repository transaction with balanced debit/credit entries and rollback coverage. | `python3 -m pytest -q afritech/tests/novapay/test_ecosystem.py` | `artifacts/phase1-core-engineering/novapay/` |  |
| NPY-OUTBOX-001 | Transactional financial outbox | Missing | VERIFIED | Financial outbox rows are normalized, transactionally written, and exercised by transfer/refund/settlement/reconciliation paths. | `python3 -m pytest -q afritech/tests/novapay/test_postgres_repository.py afritech/tests/novapay/test_ecosystem.py` | `artifacts/phase1-core-engineering/novapay/` |  |
| NPY-STATE-001 | Settlement/reversal/refund/chargeback/reconciliation state machines | Partially implemented | VERIFIED | Financial flows now publish outbox events and reuse transactional repository boundaries for the implemented states. | `python3 -m pytest -q afritech/tests/novapay` | `artifacts/phase1-core-engineering/novapay/` |  |
| NPY-RUNTIME-001 | Deterministic test shutdown | Partially implemented | VERIFIED | Broad NovaPay suite now completes cleanly under repeated execution; no new leak regressions were observed in this increment. | `python3 -m pytest -q afritech/tests/novapay` | `artifacts/phase1-core-engineering/novapay/` |  |
