"""Shared AfriPay operations used by views and async tasks."""
...
from afriride_system.django_app.apps.afripay.services import PersistentTreasuryStore
...
def party(party_id: str, country: str, kyc_level: int) -> AfriPayParty:
    party_obj, _ = AfriPayParty.objects.get_or_create(
        party_id=party_id,
...
def ensure_pool(provider: str, currency: str) -> LiquidityPool:
    pool, _ = LiquidityPool.objects.get_or_create(
        provider=provider,
...
def append_event(event_type: str, aggregate_id: str, payload: dict[str, Any]) -> EventRecord:
    previous = EventRecord.objects.order_by("-id").first()
