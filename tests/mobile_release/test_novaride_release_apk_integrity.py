...
def test_apk_validation_script_uses_integrity_and_signing_tools() -> None:
    ...
    assert "APK signing certificate does not match the NovaRide release lineage" in source


def test_public_pilot_apks_are_valid_zip_when_present() -> None:
    for apk in [
        ROOT / "apk/novaride-rider-v2026.1.4-public-pilot.apk",
        ROOT / "apk/novaride-driver-v2026.1.4-public-pilot.apk",
    ]:
        if apk.exists():
