...
def test_manifests_define_immutable_and_alias_urls() -> None:
    for name, app in [
        ("novaride_rider_v2026.1.1_manifest.json", "rider"),
        ("novaride_driver_v2026.1.1_manifest.json", "driver"),
    ]:
        manifest = json.loads((ROOT / "docs/mobile/release" / name).read_text(encoding="utf-8"))
        assert manifest["immutable_apk_url"].endswith(
            f"/novaride/releases/2026.1.1/novaride-{app}-v2026.1.1-public-pilot.apk"
        )
