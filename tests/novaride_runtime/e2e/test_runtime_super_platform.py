...
def test_rider_booking_dispatch_trip_completion_evidence_and_read_model() -> None:
    ...
    runtime.driver.onboard(
        driver_ctx, identity_id="novaid_driver_1", display_name="Driver One", vehicle_id="vehicle_1"
    )
    ...
    booking = runtime.booking.create_booking(
        ...
        BookingIntent(
            "rider_1",
            AddressRef("A", GeoPoint(-37.81, 144.96)),
            AddressRef("B", GeoPoint(-37.82, 144.97)),
            "economy",
        ),
        quote_id=quote.id,
    ...
    assert (
        runtime.booking.create_booking(
            rider_ctx,
    ...
    assert "TripEvidenceFinalized" in {
...
def test_safety_operator_fleet_logistics_corporate_transit_and_payment_boundaries() -> None:
    ...
    emergency = runtime.safety.activate_emergency(
        rider_ctx, source_id="rider_1", trip_id=None, idempotency_key="em-1"
    )
    assert emergency.evidence_locked is True
    assert (
        runtime.safety.activate_emergency(
            rider_ctx, source_id="rider_1", trip_id=None, idempotency_key="em-1"
        ).id
        == emergency.id
    )
    ...
    assert (
        runtime.fleet.compliance_hold(operator_ctx, fleet.id, "inspection_expired").compliance_hold
        is True
    )

    delivery = runtime.logistics.create_order(
        rider_ctx, "rider_1", "Recipient", AddressRef("Pickup"), AddressRef("Dropoff")
    )
    ...
    corporate_booking = runtime.corporate.create_booking(
        operator_ctx, account.id, "employee_1", "booking_ref", "CC-1"
    )
    ...
    payment = runtime.novapay.create_payment_intent_reference(
        __import__("afritech.novaride_runtime.common.money", fromlist=["Money"]).Money.of(
            "10", "AUD"
        ),
        rider_ctx,
    )
